...
public class VcfDataProvider extends DataProvider {
	...
	public VcfDataProvider(final boolean isInlineData, final String importFilePathOrData, final char[] zipPassword) {
	...
	public Map<String, Object> getNextItemData() throws Exception {
		...
		final VcfCard vcfCard = vcfReader.readNextCard();
	...
	@Override
	public void close() {
		Utilities.closeQuietly(vcfReader);
		vcfReader = null;
		super.close();
	}
	...
	public File filterDataItems(final List<Integer> indexList, final String fileSuffix) throws Exception {
		...
		try {
			...
			if (Utilities.endsWithIgnoreCase(getImportFilePath(), ".zip")) {
				filteredDataFile = new File(getImportFilePath() + "." + fileSuffix + ".vcf.zip");
				outputStream = ZipUtilities.openNewZipOutputStream(filteredDataFile, null);
				((ZipOutputStream) outputStream).putNextEntry(new ZipEntry(new File(getImportFilePath() + "." + fileSuffix + ".vcf").getName()));
			} else if (Utilities.endsWithIgnoreCase(getImportFilePath(), ".tar.gz")) {
				filteredDataFile = new File(getImportFilePath() + "." + fileSuffix + ".vcf.tar.gz");
				...
				filteredDataFile = new File(getImportFilePath() + "." + fileSuffix + ".vcf.tgz");
				tempFile = File.createTempFile(new File(getImportFilePath()).getName(), fileSuffix);
				outputStream = new FileOutputStream(tempFile);
			} else if (Utilities.endsWithIgnoreCase(getImportFilePath(), ".gz")) {
				filteredDataFile = new File(getImportFilePath() + "." + fileSuffix + ".vcf.gz");
				outputStream = new GZIPOutputStream(new FileOutputStream(filteredDataFile));
			} else {
				filteredDataFile = new File(getImportFilePath() + "." + fileSuffix + ".vcf");
				outputStream = new FileOutputStream(filteredDataFile);
			}

			try (VcfWriter vcfWriter = new VcfWriter(outputStream, encoding)) {
				VcfCard vcfCard;
				int itemIndex = 0;
				while ((vcfCard = vcfReader.readNextCard()) != null) {
					itemIndex++;
					if (indexList.contains(itemIndex)) {
						vcfWriter.writeCard(vcfCard);
					}
				}
			}
	...
	private void openReader() throws Exception {
		...
		try {
			vcfReader = new VcfReader(getInputStream());
